//! Linux-specific implementation of process context publication.
//!
...
/// - `start_addr` is non-null, is coming from a previous call to `mmap` with a size value of
///   [`mapping_size`] and hasn't been unmapped since.
...
// shares the internal pointer. It's also safe to drop (`munmap`) from a different thread.
...
impl MemMapping {
    ...
    ///
    /// `memfd` is the preferred method, but this function falls back to an anonymous mapping if
    ...
    fn new() -> Result<Self, Error> {
        ...
        .and_then(|fd| {
            ...
            // We (implicitly) close the file descriptor right away, but this is ok.
            Ok(MemMapping { start_addr })
        })
        // If any previous step failed, fall back to an anonymous mapping.
        .or_else(|_| {
    ...
    /// tried, as per the specification.
    fn set_name(&mut self) -> Result<(), Error> {
        // Safety: the invariants of `MemMapping` ensure that `start_addr` is non-null and comes
    ...
    unsafe fn unmap(&self) -> Result<(), Error> {
...
impl ProcessContextHandle {
    ...
    fn publish(payload: Vec<u8>) -> Result<Self, Error> {
        ...
        // Safety: the invariants of MemMapping ensure `start_addr` is not null and comes
        ...
        unsafe {
            // Safety: MappingHeader is packed, thus has no alignment requirement. It points
            // to a freshly mmap'd region which is valid for writing at least `mapping_size()`,
            ...
            ptr::write(
                ...
                MappingHeader {
                    ...
                    // Will be set atomically last.
                    monotonic_published_at_ns: 0,
                    payload_ptr: payload.as_ptr(),
                },
            );
            // We typically want to avoid the compiler and the hardware re-ordering the write
    ...
    fn update(&mut self, payload: Vec<u8>) -> Result<(), Error> {
        ...
        // Safety:
        //
        // [^atomic-u64-alignment]: Page size is at minimum 4KB and will always be 8-byte
        // aligned even on exotic platforms. The offset of `monotonic_published_at_ns` is 16
        // bytes, so it's 8-byte aligned (`AtomicU64` has both a size and align of 8 bytes).
        //
        // The header memory is valid for both reads and writes.
...
///
/// Then we follow the Publish protocol (allocating a fresh mapping).
///
...
mod tests {
    ...
    /// `monotonic_published_at` is not zero and that the signature is correct.
    ...
    /// Read the process context header from the current process.
    ///
    /// **CAUTION**: This reader is specialized for tests — it doesn't check for concurrent
    /// writers after reading the header.
    ...
    fn publish_then_update_process_context() {
        let payload_v1 = b"example process context payload".to_vec();
        let payload_v2 = b"another example process context payload of different size".to_vec();

        publish_raw_payload(payload_v1.clone()).expect("couldn't publish the process context");

        let header = read_process_context().expect("couldn't read back the process context");
        // Safety: the published context must have put valid bytes of size payload_size.
        let read_payload =
        ...
        assert_eq!(read_payload, payload_v1.as_slice(), "payload mismatch");

        let published_at_ns_v1 = published_at;
        // Ensure the clock advances so the updated timestamp is strictly greater.
        std::thread::sleep(std::time::Duration::from_millis(1));

        publish_raw_payload(payload_v2.clone()).expect("couldn't update the process context");
        ...
        assert_eq!(read_payload, payload_v2.as_slice(), "payload mismatch");
    ...
    fn unpublish_process_context() {
        let payload = b"example process context payload".to_vec();

        publish_raw_payload(payload).expect("couldn't publish the process context");
