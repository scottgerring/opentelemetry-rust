    ...
    RT: RuntimeChannel,
{
    ...
    /// Configures the runtime to use for the [PeriodicReader].
    ///
    ...
    pub fn with_runtime<R: RuntimeChannel>(self, runtime: R) -> PeriodicReaderBuilder<E, R> {
        PeriodicReaderBuilder {
...
///
/// // Using the default NoAsync runtime (backward compatible)
/// let reader = PeriodicReader::builder(exporter).build();
/// # drop(reader);
/// # }
/// ```
///
/// To use a specific async runtime like Tokio:
///
/// ```no_run
/// use opentelemetry_sdk::metrics::PeriodicReader;
...
impl<E: PushMetricExporter> PeriodicReader<E, NoAsync> {
    /// Configuration options for a periodic reader using the default [`NoAsync`] runtime.
    ///
    /// This is the recommended way to create a `PeriodicReader` for most use cases.
    /// For async runtimes like Tokio, use [`with_runtime`](PeriodicReaderBuilder::with_runtime)
    /// on the builder.
    ///
    /// [`NoAsync`]: crate::runtime::NoAsync
    pub fn builder(exporter: E) -> PeriodicReaderBuilder<E, NoAsync> {
        PeriodicReaderBuilder::new(exporter, NoAsync)
    }
}
...
impl<E: PushMetricExporter, R: RuntimeChannel> PeriodicReader<E, R> {
    ...
    pub fn builder_with_runtime(exporter: E, runtime: R) -> PeriodicReaderBuilder<E, R> {
...
mod tests {
        ...
        RT: crate::runtime::RuntimeChannel,
    {
        ...
        let exporter = InMemoryMetricExporter::default();
        let reader = PeriodicReader::builder_with_runtime(exporter.clone(), runtime)
